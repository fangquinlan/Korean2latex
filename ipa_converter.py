...
def initialize(tables_dir: Path = Path('tables')):
    initialize_conversion_tables(tables_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='韩语转国际音标（IPA）和拼音转换器，并生成逐词对照格式的文本')
    parser.add_argument('hangul', type=str, nargs='*', help='要转换的韩语文本')
    parser.add_argument('-r', '--rules', type=str, default='pastcnhovr', help='要应用的规则（默认: pastcnhovr）')
    parser.add_argument('-c', '--convention', type=str, default='ipa', help='转录规范（默认: ipa）')
    parser.add_argument('-s', '--sep', type=str, default='', help='分隔符（默认: 空字符串）')
    parser.add_argument('-f', '--file', type=str, help='从文件中读取韩语文本')
    parser.add_argument('-p', '--pinyin', action='store_true', help='仅输出拼音')
    parser.add_argument('-b', '--both', action='store_true', help='同时输出 IPA 和拼音')
    parser.add_argument('-o', '--output', type=str, help='输出文件路径')
    args = parser.parse_args()

    # Determine output mode based on user options
    if args.both:
        output_mode = 'both'
    elif args.pinyin:
        output_mode = 'pinyin'
    ...
    if not tables_dir.exists() or not tables_dir.is_dir():
        print(f"错误: 无法找到 'tables' 文件夹。请确保 'tables' 文件夹存在并包含所有必要的 CSV 文件。")
        sys.exit(1)

    try:
        initialize(tables_dir)
    ...
    if args.file:
        ...
        try:
            with open(file_path, 'rb') as f:
                content = f.read()
            # 将文件内容进行 base64 编码以适配 convert_many 函数
            encoded_content = b64encode(content).decode('utf-8')
            # 调用 convert_many 并传递 output_mode
            result = convert_many(encoded_content,
                                  rules_to_apply=args.rules,
        ...
        # 从命令行参数获取韩语文本
        if not args.hangul:
            print("请输入要转换的韩语文本。例如:")
            print("  python ipa_converter.py 안녕하세요")
            sys.exit(0)
        input_text = ' '.join(args.hangul)
        try:
            converted_results = convert(hangul=input_text,
                                        rules_to_apply=args.rules,
                                        convention=args.convention,
                                        sep=args.sep,
                                        tables_dir=tables_dir)
            ipa_result = converted_results["ipa"]
            pinyin_result = converted_results["pinyin"]

            if output_mode == 'both':
                anno_ipa = f'\\anno{{{input_text}}}{{{ipa_result}}}'
                anno_pinyin = f'\\anno{{{input_text}}}{{{pinyin_result}}}'
                output_content = f'{anno_ipa}\n{anno_pinyin}'
            elif output_mode == 'pinyin':
                anno_pinyin = f'\\anno{{{input_text}}}{{{pinyin_result}}}'
                output_content = anno_pinyin
            else:
                anno_ipa = f'\\anno{{{input_text}}}{{{ipa_result}}}'
                output_content = anno_ipa
        except Exception as e:
            print(f"转换过程中出现错误: {e}")
            sys.exit(1)

    # 如果指定了输出文件，则写入文件，否则打印到屏幕
    if args.output:
        try:
            with open(args.output, 'w', encoding='utf-8') as f:
                f.write(output_content + '\n')
            print(f"转换结果已写入到 {args.output}")
        except Exception as e:
            print(f"写入文件时出错: {e}")
            sys.exit(1)
    else:
